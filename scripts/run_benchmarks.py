...
from bidirectional_inverse.benchmark import BenchmarkConfig, benchmark_fixed_n_vary_s_to_csv, benchmark_to_csv
...
def parse_args():
    ...
    parser.add_argument("--sweep", choices=["n", "s"], default="n", help="Sweep matrix size n or sparsity s.")
    parser.add_argument("--sampling-mode", choices=["ordered", "random"], default="ordered")
    parser.add_argument("--fixed-n", type=int, default=500, help="Fixed n used when --sweep s.")
    parser.add_argument("--min-s", type=int, default=2, help="Minimum sparsity used when --sweep s.")
    parser.add_argument("--max-s", type=int, default=500, help="Maximum sparsity used when --sweep s.")
...
def build_sparsity_samples(args) -> np.ndarray:
    rng = np.random.default_rng(args.seed)
    if args.sampling_mode == "ordered":
        samples = np.logspace(np.log10(args.min_s), np.log10(args.max_s), num=args.num_samples)
        return np.unique(np.round(samples).astype(int))

    samples = rng.integers(low=args.min_s, high=args.max_s + 1, size=args.num_samples)
    return np.round(samples).astype(int)


def main():
    args = parse_args()
    samples = build_samples(args) if args.sweep == "n" else build_sparsity_samples(args)
    ...
    if args.sweep == "n":
        ...
        if not df.empty:
            df_sweep = df[df["n"] == args.fixed_n]
            generate_sparsity_plots(df_sweep, args.artifacts_dir)
